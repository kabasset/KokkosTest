...
{
  Linx::Position<int, 2> in("in", {0, 1});
  Linx::Position<int, 2> delta("delta", {-1, 1});
  
  auto plus = in + delta;
  BOOST_TEST((plus == Linx::Position<int, 2> {-1, 2})); // FIXME == {-1, 2}
  
  auto minus = in - delta;
  BOOST_TEST((minus == Linx::Position<int, 2> {1, 0})); // FIXME == {1, 0}
}
